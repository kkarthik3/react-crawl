...
const TestDriveForm: React.FC<TestDriveFormProps> = ({ onSubmit, onCancel }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const selectedShowroom = showrooms?.find(showroom => showroom.id === formData.branch);
    ...
    const startDateTime = parse(formData.startTime, 'hh:mm a', new Date(formData.date));
    const endDateTime = parse(formData.endTime, 'hh:mm a', new Date(formData.date));
  ...
  return (
    <div className="max-w-72 mx-auto p-2 space-y-1 bg-white rounded-lg shadow-lg">
      ...
      >
        ...
        <div className="flex flex-col space-y-1">
          ...
          <select
            ...
            onChange={(e) => setFormData((prev) => ({ ...prev, branch: e.target.value, model: '', vehicle: '' }))}
        ...
        {/* Test Drive Date */}
        <div className="flex flex-col space-y-1">
          <label className="text-xs font-medium text-gray-700">Test Drive Date</label>
          <input
            type="date"
        ...
        <div className="flex flex-col space-y-1">
          ...
          <select
            ...
            onChange={(e) =>
              setFormData((prev) => ({ ...prev, model: e.target.value, vehicle: '' }))
            }
            className="w-full px-2 py-1 border border-gray-300 rounded-md shadow-sm text-xs"
          >
        ...
        <button
          type="submit"
          className="w-full bg-blue-500 text-white text-xs font-medium py-2 rounded-md hover:bg-blue-600"
        >
          Book Test drive
        </button>
      </form>
